...
Player::Player(shared_ptr<Mesh> mesh, Material material,
               ...
               GameObject *parent)
    : GameObject(mesh, material, parent, false, 2), bulletMesh(bulletMesh),
      mapMinLimit(mapMinLimit), mapMaxLimit(mapMaxLimit) {
...
void Player::update() {
  ...
  if (rigidbody.velocity != glm::vec3(0, 0, 0)) {
    footstepsSound.setPitch(keyboard[SDL_SCANCODE_LSHIFT] ? 1.5f : 1.0f);
  ...
  rigidbody.velocity *= keyboard[SDL_SCANCODE_LSHIFT] ? runningSpeed : speed;
  int mouseX, mouseY;
...
void Player::dispatchBullet() {
  gunShotSound.play();
  bullets.push_back(Bullet(bulletMesh, Material(), nullptr,
                           vec3(0, 0, -1))); // TODO custom mat
  bullets[bullets.size() - 1].transform = transform;
  bullets[bullets.size() - 1].transform.translate(
      {0.249067f, 0.47149f, -1.25759f});
  isRecoiling = true;
  recoilTime = 0.0f;
