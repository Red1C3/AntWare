...
void Window::init(int height, int width) {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    printf("Failed to init SDL_Video, err: %s\n", SDL_GetError());
    assert(0);
  }
#else
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0) {
    printf("Failed to init SDL_Video+Audio, err: %s\n", SDL_GetError());
    assert(0);
  }
#endif
  ...
  internal.window = SDL_CreateWindow("AntWare", SDL_WINDOWPOS_UNDEFINED,
...
SDL_Window *Window::WindowAdapter::getHandle() { return window; }
int Window::WindowAdapter::pollEvent(SDL_Event &event) {
  return SDL_PollEvent(&event);
}
void Window::WindowAdapter::display() { SDL_GL_SwapWindow(window); }
